...
function applyPublishedPromotions() {
  ...
  HS_DATA.promotions.forEach((promotion, index) => {
    ...
    const links = slide.querySelectorAll(".hero-cta a"); links[0].textContent=promotion.primary_label; links[0].href=promotion.primary_url; links[1].textContent=promotion.secondary_label; links[1].href=promotion.secondary_url;
    ...
    const image=slide.querySelector(".hero-product-image"); image.src=promotion.image_url; image.alt=promotion.image_alt;
  });
}
...
function initCatalogo() {
  ...
  filters.innerHTML = cats.map(c =>
    `<button class="chip-filter ${c === "Todos" ? "is-active" : ""}" data-cat="${c}">${c}</button>`).join("");
...
function initProducto() {
  ...
  wrap.innerHTML = `
    <div class="product-media">
      ${p.imageUrl ? `<img class="product-image product-image--detail" src="${p.imageUrl}" alt="${p.name}">` : `<svg class="product-illu" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1">${hsIcon(p.icon).match(/<svg[^>]*>([\s\S]*)<\/svg>/)[1]}</svg>`}
    </div>
    <div>
      <span class="product-cat">${p.category} · ${p.id}</span>
      <h1 style="font-size:1.8rem;margin:8px 0 6px">${p.name}</h1>
      ...
      <p class="text-muted mt-2">${p.desc}</p>
...
function initCarrito() {
  ...
  function render() {
    ...
    listEl.innerHTML = `<div class="card">` + items.map(i => `
      <div class="cart-item" data-line="${i.id}">
        <a class="cart-thumb" href="producto.html?id=${i.id}">${i.imageUrl ? `<img src="${i.imageUrl}" alt="${i.name}">` : hsIcon(i.icon)}</a>
        <div>
          <h4><a href="producto.html?id=${i.id}" style="color:inherit">${i.name}</a></h4>
          <div class="unit">${hsMoney(i.price)} c/u · ${i.category}</div>
