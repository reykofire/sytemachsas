...
const palette = { ink: "#16232f", red: "#c71920", gray: "#687681", pale: "#f5f7f8", line: "#dfe5e8" };
...
async function sendTicketMail(ticket: TicketMail, event: "created" | "closed", recipients = [ticket.requester_email], staff = false) {
  ...
  await transport.sendMail({ from: config.SMTP_FROM, to: primaryRecipient, bcc: hiddenRecipients, subject: `${staff ? "Nueva solicitud" : event === "created" ? "Solicitud recibida" : "Solicitud cerrada"} · ${ticketCode(ticket)} · HardSystem`, html: staff ? staffCreatedHtml(ticket) : event === "created" ? createdHtml(ticket) : closedHtml(ticket), text: `${staff ? "Nueva solicitud" : event === "created" ? "Solicitud recibida" : "Solicitud cerrada"}: ${ticketCode(ticket)} - ${ticket.subject}. Consulta el portal HardSystem.`, attachments: [{ filename: "logo-hardsystem.png", path: "/app/Logo/logoHS01.png", cid: "hardsystem-logo" }] });
  return true;
}
...
export async function notifyContactRequest(request: ContactMail, recipients: string[]) {
  if (!transport || !recipients.length) return false;
  const [primaryRecipient, ...hiddenRecipients] = recipients;
  await transport.sendMail({ from: config.SMTP_FROM, to: primaryRecipient, bcc: hiddenRecipients, replyTo: request.email, subject: `Nueva solicitud de atención · ${request.service} · HardSystem`, html: staffContactHtml(request), text: `Nueva solicitud de ${request.name} (${request.email}) sobre ${request.service}: ${request.message}`, attachments: [{ filename: "logo-hardsystem.png", path: "/app/Logo/logoHS01.png", cid: "hardsystem-logo" }] });
  return true;
}
export async function notifyContactConfirmation(request: ContactMail) {
  if (!transport) return false;
  await transport.sendMail({ from: config.SMTP_FROM, to: request.email, subject: `Solicitud recibida · HardSystem`, html: contactHtml(request), text: `Hola ${request.name}, recibimos tu solicitud sobre ${request.service}. Te contactaremos pronto.`, attachments: [{ filename: "logo-hardsystem.png", path: "/app/Logo/logoHS01.png", cid: "hardsystem-logo" }] });
  return true;
}
