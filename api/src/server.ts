...
app.get("/api/reports/statistics.pdf", { preHandler: auth }, async (request, reply) => {
  ...
  const doc = new PDFDocument({ size: "A4", margin: 42, info: { Title: "Estadísticas operativas - Systemach", Author: "Systemach" } }); const chunks: Buffer[] = []; doc.on("data", chunk => chunks.push(chunk)); const complete = new Promise<Buffer>((resolveDocument, reject) => { doc.on("end", () => resolveDocument(Buffer.concat(chunks))); doc.on("error", reject); });
  const navy = "#16232f"; const red = "#006eae"; const gray = "#687681"; const line = "#dfe5e8"; const pale = "#f5f7f8"; const logoPath = resolve(process.cwd(), "Logo/logo-systemach.png");
  try { doc.image(logoPath, 42, 20, { fit: [90, 64] }); } catch { doc.font("Helvetica-Bold").fontSize(15).fillColor(navy).text("Systemach", 42, 43); } doc.font("Helvetica-Bold").fontSize(17).fillColor(navy).text("ESTADÍSTICAS OPERATIVAS", 180, 42, { width: 373, align: "right" }); doc.font("Helvetica").fontSize(8.5).fillColor(gray).text(`Periodo: ${filters.period}  ·  Generado ${new Intl.DateTimeFormat("es-CO", { dateStyle: "medium", timeStyle: "short" }).format(new Date())}`, 180, 67, { width: 373, align: "right" }); doc.moveTo(42, 88).lineTo(553, 88).strokeColor(red).lineWidth(2).stroke();
...
app.get("/api/reports/collection/:type.pdf", { preHandler: auth }, async (request, reply) => {
  ...
  const doc = new PDFDocument({ size: "A4", margin: 42, info: { Title: `${definition.title} - Systemach`, Author: "Systemach" } });
  const chunks: Buffer[] = [];
  ...
  doc.font("Helvetica").fontSize(8).fillColor(gray).text("Documento generado por Systemach  ·  Registro operativo confidencial", 42, 770, { width: 511, align: "center" });
  doc.end();
...
app.get("/api/reports/collection/:type/:id.pdf", { preHandler: auth }, async (request, reply) => {
  ...
  const doc = new PDFDocument({ size: "A4", margin: 42, info: { Title: `Expediente ${type} - Systemach`, Author: "Systemach" } });
  const chunks: Buffer[] = []; doc.on("data", chunk => chunks.push(chunk));
  const complete = new Promise<Buffer>((resolveDocument, reject) => { doc.on("end", () => resolveDocument(Buffer.concat(chunks))); doc.on("error", reject); });
  const navy = "#16232f"; const red = "#006eae"; const gray = "#687681"; const line = "#dfe5e8"; const pale = "#f5f7f8"; const green = "#177245";
  ...
  doc.font("Helvetica").fontSize(8).fillColor(gray).text("Documento generado por Systemach  ·  Expediente operativo confidencial", 42, 770, { width: 511, align: "center" }); doc.end();
...
app.get("/api/reports/:id.pdf", { preHandler: auth }, async (request, reply) => {
  ...
  const doc = new PDFDocument({ size: "A4", margin: 42, info: { Title: `Reporte ${ticket.ticket_number} - Systemach`, Author: "Systemach" } });
  const chunks: Buffer[] = [];
  doc.on("data", chunk => chunks.push(chunk));
  const complete = new Promise<Buffer>((resolveDocument, reject) => { doc.on("end", () => resolveDocument(Buffer.concat(chunks))); doc.on("error", reject); });
  const navy = "#16232f"; const red = "#006eae"; const gray = "#687681"; const line = "#dfe5e8"; const pale = "#f5f7f8"; const green = "#177245";
  ...
  const logoPath = resolve(process.cwd(), "Logo/logo-systemach.png");
  try { doc.image(logoPath, 42, 20, { fit: [90, 64] }); } catch { doc.font("Helvetica-Bold").fontSize(18).fillColor(navy).text("Systemach", 42, 48); }
  ...
  doc.addPage();
  try { doc.image(logoPath, 42, 20, { fit: [90, 64] }); } catch { doc.font("Helvetica-Bold").fontSize(14).fillColor(navy).text("Systemach", 42, 43); }
